...
class MinibarConsumption(Base):
    ...
    product_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("minibar_products.id"))
    quantity: Mapped[int] = mapped_column(SmallInteger)
    unit_price_pen: Mapped[float] = mapped_column(Numeric(8, 2))
    unit_price_usd: Mapped[float] = mapped_column(Numeric(8, 2))
    total_pen: Mapped[float] = mapped_column(Numeric(8, 2))
    total_usd: Mapped[float] = mapped_column(Numeric(8, 2))
